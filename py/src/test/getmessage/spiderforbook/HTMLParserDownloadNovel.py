...
@author: hasee
test.getmessage.spiderforbook.HTMLParserDownloadNovel
'''
# coding:utf-8
'''
Created on 2016年12月7日
...
class HTMLParserDownloadBook(HTMLParser):
    def __init__(self):
        ...
        if tag == 'a' and attrs:
            for (attr, value) in attrs:
                ...
                if attr == 'class' and value == 'next':
                    ...
                    print attrs
                    # 匹配完整的url
                    self.chapternextlink = self.format_url(self.chapternextlink, attrs[0][1])
...
#                     print 'in content'
    ...
    def format_url(self, sourceurl, nexturl):
        print len(sourceurl.split('/'))
        print len(nexturl.split('/')) 
        newurl = ''
        for u in sourceurl.split('/')[0:-1]:
            newurl += u + '/'
        
        newurl += nexturl.split('/')[-1]
        print newurl
        return newurl
...
if __name__ == '__main__':
    firstchapter = 'http://www.quanshu.net/book/107/107337/28157524.html'
    endurl = 'http://www.quanshu.net/book/107/107337/index.html'
